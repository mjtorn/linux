...
static void __sched notrace __schedule(unsigned int sched_mode)
{
	...
	if (rq->preempt) {
		 ...
		 */
		if (unlikely((sched_mode & SM_MASK_PREEMPT) && prev != idle && !test_tsk_need_resched(prev))) {
			rq->preempt = NULL;
...
void __init init_idle(struct task_struct *idle, int cpu)
{
	...
	WRITE_ONCE (idle->__state, TASK_RUNNING);
...
#ifdef CONFIG_SYSCTL
static struct ctl_table sched_muqss_sysctls[] = {
	{
...
static int __init sched_muqss_sysctl_init(void)
{
	register_sysctl_init("kernel", sched_muqss_sysctls);
	return 0;
}
late_initcall(sched_muqss_sysctl_init);
#endif /* CONFIG_SYSCTL */
